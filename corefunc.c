...
pl_analogRead(term_t pin_,term_t value){
	int pin,retval;
	if(!PL_get_integer(pin_,&pin)){
		return PL_warning("Instantiation fault in analogRead/2. Pin number should be an interger.");
	}
...
pl_analogWrite(term_t pin_,term_t value_){
	int pin,value;
	if(PL_get_integer(pin_,&pin)){
		return PL_warning("Instantiation fault in analogWrite/2. Pin number should be an integer.");
	}
	if(PL_get_integer(value_,&value)){
		return PL_warning("Instantiation fault in analogWrite/2. Pin number should be an integer.");
	}
